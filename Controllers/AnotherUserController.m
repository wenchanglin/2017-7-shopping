...
@interface AnotherUserController ()<UITableViewDelegate,UITableViewDataSource> {
    
}
@property (nonatomic, strong) UITableView *tableView;
@property (nonatomic, weak) AnotherTopView *header;
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if ([result[@"errcode"] integerValue] == 0) {
                if (NotNilAndNull(result[@"data"])) {
                    ...
                    [self.header fillMyInformationWith:data];
                     [self.view addSubview:self.tableView];
                     [self.view addSubview:self.leview];
                }
            }
...
- (AnotherTopView *)header {
    
    if (!_header) {
        _header = [[NSBundle mainBundle] loadNibNamed:@"AnotherTopView" owner:nil options:nil].lastObject;
        _header.frame = CGRectMake(0, 0, kScreenSize.width, 360.f);
    }
    return _header;
}
